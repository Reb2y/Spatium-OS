(function initSpatiumOS() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const interactiveSelector = 'button, a, input, .desktop-icon, .start-item, .cal-day-cell, .cal-nav-btn, .start-btn, .taskbar-time, .window-close-btn';
        ...
        // БАЗА ДАННЫХ ПРАЗДНИКОВ (Формат: "ММ-ДД")
        const holidaysData = {
            "01-01": "Новый год",
        ...
        function switchToDesktop() {
            ...
            setTimeout(() => {
                ...
                // ВКЛЮЧАЕМ КУРСОР НА РАБОЧЕМ СТОЛЕ
                document.body.classList.add('show-cursor');
            }, 1500);
        }

        function switchToTerminal() {
            // ОТКЛЮЧАЕМ КУРСОР В КОНСОЛИ
        ...
        function shutdownSystem() {
            ...
            if (screen) screen.classList.add('crt-off');
            setTimeout(() => {
                document.body.innerHTML = '<div style="background:#000;width:100vw;height:100vh;display:flex;justify-content:center;align-items:center;color:#00ff00;font-family:sans-serif;">ПИТАНИЕ КОМПЬЮТЕРА ОТКЛЮЧЕНО. МОЖНО ЗАКРЫТЬ ВКЛАДКУ.</div>';
            }, 800);
        }
        ...
        window.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#desktopContainer')) return;
  ...
  off                - Выключение терминала`,
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (isHackerMode) { stopHackerMode(); e.preventDefault(); return; }
