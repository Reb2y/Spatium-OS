(function initSpatiumOS() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // ПОДДЕРЖКА ВВОДА С МОБИЛЬНОЙ КЛАВИАТУРЫ
        if (terminalContainer) {
            terminalContainer.addEventListener('click', () => {
                if (mobileInput && isBooted && desktopContainer.classList.contains('hidden')) {
                    mobileInput.focus();
                }
            });
        }
        ...
        if (settingsHeader && settingsWindow) {
            ...
            // Поддержка Touch Drag на смартфонах
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowUp') {
                ...
                if (commandHistory.length > 0 && historyIndex > 0) {
                    ...
                    if (commandInputText) commandInputText.textContent = currentInput;
                    if (mobileInput) mobileInput.value = currentInput;
                }
                return;
            }
